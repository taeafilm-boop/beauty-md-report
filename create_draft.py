...
# 2. 뷰티 기사 검증용 키워드 필터 정의
BEAUTY_MUST_KEYWORDS = [
    "화장품", "뷰티", "스킨케어", "메이크업", "코스메틱", "더마", "쿠션", "앰플", "세럼", 
    "크림", "선크림", "선케어", "립", "틴트", "마스크팩", "패드", "클렌징", "올리브영", 
    "아모레", "LG생활건강", "LG생건", "설화수", "라네즈", "헤라", "에스트라", "더후", 
    "CNP", "피지오겔", "메디큐브", "클리오", "달바", "롬앤", "토리든", "넘버즈인", 
    "마녀공장", "닥터지", "라운드랩", "아누아", "에이지알", "에스쁘아", "미샤", "이니스프리"
]

NON_BEAUTY_EXCLUDE_KEYWORDS = [
    "게임", "패키지 게임", "화이트데이", "반도체", "소득", "최빈국", "신발", "가발", 
    "ETF", "증시", "코스피", "코스닥", "주가", "부동산", "아파트", "사설", "칼럼", 
    "이성의 빛", "오늘 하루", "정치", "대통령", "국회", "스포츠", "축구", "야구", "골프"
]

# 3. 기사 페이지에서 실제 1~2줄 요약문(og:description) 추출
...
# 4. 실시간 뷰티 전용 구글 뉴스 RSS 크롤링 (엄격한 이중 필터링)
def fetch_cosmetic_news():
    # 1차: 구글 뉴스 검색 쿼리에서 필수 뷰티 단어 한정 및 비뷰티 제외어 적용
    query = (
        '("화장품" OR "뷰티" OR "스킨케어" OR "메이크업" OR "더마") '
        '(설화수 OR 라네즈 OR 에스트라 OR 헤라 OR 더후 OR CNP OR 피지오겔 OR 메디큐브 OR '
        '달바 OR 클리오 OR 롬앤 OR 토리든 OR 넘버즈인 OR 마녀공장 OR 닥터지 OR 올리브영 OR '
        '신제품 OR 출시 OR 신상 OR 완판 OR 랭킹 OR 쿠션 OR 앰플 OR 세럼 OR 크림 OR 립) '
        '-게임 -반도체 -증시 -ETF -소득 -최빈국 when:2d'
    )
    ...
    try:
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            for item in items:
                ...
                # [2차 필터]: 뷰티 필수 키워드가 하나도 없으면 스킵
                if not any(k in title for k in BEAUTY_MUST_KEYWORDS):
                    continue
                    
                # [3차 필터]: 비(非)뷰티 제외 키워드가 포함되어 있으면 스킵
                if any(k in title for k in NON_BEAUTY_EXCLUDE_KEYWORDS):
                    continue
...
# 5. 요약 및 11번가 MD 인사이트 생성
def generate_insights(articles):
    if GEMINI_API_KEY:
        try:
            prompt = """당신은 11번가 뷰티 카테고리 전문 MD입니다. 아래 뷰티/화장품 브랜드 및 상품 뉴스 5건을 분석하여 다음 규칙을 지켜 응답해주세요:
1) summary: 브랜드 및 상품명, 핵심 스펙/효능 중심 2줄 요약 (문장 앞에 • 포함, 줄바꿈은 <br>. 'Google News' 등 영문 시스템 문구 제외)
2) insight: 11번가 뷰티 MD 관점의 상품 소싱/단독 구성/프로모션/크로스셀링 전략 2줄 (문장 앞에 • 포함, 줄바꿈은 <br>. 5개 기사 모두 내용이 겹치지 않게 작성)
...
"""
    ...
    for idx, a in enumerate(articles):
        ...
        if desc and len(desc) > 20:
            ...
            if any(k in t_lower for k in ["에디션", "콜라보", "한정판", "헬로키티"]):
            ...
            elif any(k in t_lower for k in ["디바이스", "메디큐브", "기기", "에이지알"]):
                a["summary"] = f"• {clean_title}.<br>• 홈케어 뷰티 디바이스와 전용 기능성 앰플 결합을 통한 안티에이징 수요 선점."
            elif any(k in t_lower for k in ["립", "틴트", "센슬"]):
...
if not articles:
    raise Exception("뷰티 뉴스를 크롤링하지 못했습니다.")
articles = generate_insights(articles)

# 7. HTML 카드 생성 (완전한 화이트 배경 및 11번가 서체)
cards_html = ""
for idx, a in enumerate(articles):
...
for folder in candidate_folders:
    ...
    if status == 'OK':
        print(f"성공: [{folder}] 폴더에 100% 뷰티 전용 리포트 초안이 정상 생성되었습니다.")
        success = True
        break
